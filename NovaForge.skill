...
将任意学科/项目的知识体系整理为结构化笔记。支持 Typst（默认，编译 PDF）、LaTeX（编译 PDF）和 Markdown（快速记录）三版本模板，适用于考研、考公、专业课学习、科研研究、项目总结、竞赛准备等场景。

**核心理念：** 先理解知识体系 → 按模块组织 → 生成含概念+原理+方法+案例+实战+练习+复盘的完整笔记。

## Non-Negotiables

- 支持三种输出格式，用 AskUserQuestion 让用户选择（如果用户没明确说用哪个）：**Typst**（编译 PDF，默认推荐，语法现代、单次编译，公式用 LaTeX 风格行内 `$...$` 由 Typst 渲染）、**LaTeX**（编译 PDF，公式/TikZ 重度或期刊模板时备选）、**Markdown**（纯文本即时记录）。
...
- 默认编译方式：Typst 模式 `typst compile` 单次编译（首选），LaTeX 模式 `xelatex` × 2 遍。
...
- 不编造个人信息：姓名/学号/班级/真题来源/数据/文献一律不得虚构；仅当题干或用户明确要求署名时才填真名（"黎达宏"），学号默认留空、仅用户明确要求时才写。
- 去 AI 痕迹：成稿与编译日志中不得出现本机绝对路径、临时目录、脚本名或工具名（OfficeCLI/Codex/ChatGPT/Claude 等），日志路径写成普通课程相对路径；不写入 docProps 之类元数据痕迹。
- 朴素克制版式（作业类成稿默认生效）：正文黑色宋体、标题居中加粗即可；不主动加蓝色/主题色字、彩色底纹标题栏、装饰性横线分隔线、复杂底纹。彩色配色仅在用户明确要求美化时启用。

## Required Outcome

默认成功结果：

- Typst 模式（默认）：保存 `.typ` 源文件 + 编译后的 `.pdf`
- LaTeX 模式：保存 `.tex` 源文件 + 编译后的 `.pdf`
- Markdown 模式：保存 `.md` 文件
- 所有生成文件放入 `NovaForge-Output-<project-name>/` 文件夹
- 最终回复报告生成的文件路径
- 同一需求多次生成时，文件名/回复须标明最终版与中间版（如 `<name>-notes-v2` / 末尾注"最终版"），避免新旧混淆
...
- 确认用户要整理什么学科/项目
- 确认用 Typst（编译 PDF，默认推荐）、LaTeX（编译 PDF，备选）还是 Markdown（纯文本）——用户没说则用 AskUserQuestion 弹出选择，默认项指向 Typst
- 确认输出路径（默认当前工作目录下的 `NovaForge-Output-<project-name>/`）
- 确认覆盖范围（全部内容还是指定章节）；缩小范围或排除项以用户原话为准，逐字遵守；用户说"不用补了/就这些"即停，不擅自扩写或补章节
- **确认模板模式**：
...
- tone：清晰、结构化、不啰嗦
- output：`.typ` + `.pdf`（Typst 模式，默认）、`.tex` + `.pdf`（LaTeX 模式）或 `.md`（Markdown 模式），放入 `NovaForge-Output-<project-name>/`

## Step 2. Check Build Environment

- Typst 模式（默认）：确认 `typst` 可用（安装方式：`cargo install typst-cli` 或从 https://github.com/typst/typst/releases 下载）
- LaTeX 模式：确认 `xelatex` 可用
- 如果 `typst`/`xelatex` 缺失但有 PDF 需求，报告环境问题
- 中文字体缺失（PDF 出现方框/豆腐块）优先在本机修复——检查并安装/指定本机可用中文字体（如宋体/思源宋体），Typst 用 `#set text(font: ...)`、LaTeX 用 `\setCJKmainfont` 指向本机实际可用字体，而不是放弃 PDF 或换成非中文字体
...
- `创新点` — 理论/方法/应用层面的创新贡献
- `局限性` — 方法局限、未解决问题、争议点
- `与己关联` — 默认锚定用户真实背景（海洋科学/鱼类科研/生物信息学/Python 与 AI agent 开发），写出基于材料的具体可迁移点或不可迁移差异，非通用感悟（详见科研模式特有规则）
...
│   方法局限 → 未解决问题 → 改进方向
├─ 六、与自身研究的关联
│   可迁移点 → 可复用的工具/方法 → 不可迁移差异（默认锚定用户背景，见科研模式特有规则）
└─ 附录
...
**通用要求：**
- 凝练（硬要求）：短句、要点式；广度优先、深度按需；严禁照搬教材原句或写成论文综述式长段；同一意思不重复表达，可删则删
...
- 避免长篇大论
- 不照搬教材原句，用自己的话凝练改写
- 三料合并：课程心得/小报告须把"题干要求 + 用户真实背景 + 课程核心概念"三料合并，拒绝通用感悟或论文综述式铺陈
- 列举数量核对：题干指定"N 个/N 条/N 点"时，正文必须显式编号 `1./2./3.` 且条数与 N 完全一致；verifier 阶段逐条数清，多删少补
- 独立见解须有据：凡要求"谈谈看法/个人分析/课程心得"，结论必须基于材料中的具体证据（数据、图、原文论点），不写脱离材料的空泛感想或套话
...
- 日期放在封面作者署名下方，与正文同色（黑色常规字），不另加彩色或装饰，格式如：`最后修订：2026年5月14日`。
...
- "与自身研究的关联" 默认锚定用户真实背景——海洋科学/鱼类科研/生物信息学/Python 与 AI agent 开发：须基于材料写出该方法、数据或思路能否迁移到用户自己的鱼类/海洋数据分析或工具开发上，每条关联落到一个可操作迁移点或一处不可迁移差异，禁止写成"对相关领域有启发"这类通用空泛感悟；用户另指定方向时以用户为准
...
- 最终修订日期与正文同色，不另加彩色或装饰；文末祝福区同色处理
...
- 指定数量的列举项是否齐全且显式编号（题干要求"N 个/N 条/N 点"时逐条数清，与 N 完全一致）
- 公式是否准确
- 内容是否有明显错误
- 排版是否清晰
...
**默认朴素模式（作业类成稿优先）：** 仅用黑色正文 + 至多一种深色（深蓝或深灰）用于标题与公式框，不上多色系；横线/分隔线/底纹仅用于必要的表格与公式框，不做装饰性使用。上面的八色系统降为可选，仅在用户明确要求"多彩/醒目/美化"时启用。
...
│   方法局限 → 未解决问题 → 改进方向
├─ \section{六、与自身研究的关联}
│   可迁移点 → 可复用方法/工具 → 不可迁移差异（默认锚定用户背景，见科研模式特有规则）
└─ \section{参考文献}
    完整引用信息
```
...
- Typst 模式（默认）：`typst compile <filename>.typ` 编译
- LaTeX 模式：`xelatex` × 2 遍编译
- Markdown 模式：无需编译，确认 `.md` 已写入磁盘
- 确认 `.typ` / `.tex` / `.md` 已写入磁盘
- 确认 `.pdf` 编译成功（exit status = 0）
- 检查最终 PDF 是否有：乱码、公式错误、排版问题、中文渲染失败
- 页数与分页不靠估算——必须以渲染出的 PDF 实际页数为准（编译后读 PDF 页数/用 PDF 工具计数）；用户对页数或分页敏感（如限页作业）时以目标渲染器结果为准，不要在回复里报"大约 N 页"
- 去 AI 痕迹复核：编译日志与回复中不出现本机绝对路径、临时目录、脚本名或工具名（OfficeCLI/Codex/ChatGPT/Claude 等），路径写成普通课程相对路径
